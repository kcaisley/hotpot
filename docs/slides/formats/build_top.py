# build_top.py -- same two-cell placement/routing as layout/build_layout.py.
from pathlib import Path
import json
P=Path(__file__).resolve().parent;G=json.loads((P.parent/'layout/geometry.json').read_text())
lines=['# top.def','VERSION 5.8 ;','DIVIDERCHAR "/" ;','BUSBITCHARS "[]" ;','DESIGN top ;','UNITS DISTANCE MICRONS 2000 ;','DIEAREA ( -600 -400 ) ( 2600 3400 ) ;','ROW ROW_0 FreePDK45_38x28_10R_NP_162NW_34O 0 0 N DO 5 BY 1 STEP 380 0 ;','TRACKS Y 140 DO 10 STEP 280 LAYER metal1 ;','TRACKS X 190 DO 5 STEP 380 LAYER metal2 ;','COMPONENTS 2 ;','- inst1 NAND2_X1 + PLACED ( 0 0 ) N ;','- inst2 INV_X1 + PLACED ( 1140 0 ) N ;','END COMPONENTS','PINS 3 ;']
...
for name,pts in G['routes_um'].items():
 ...
 lines[-1]+=' ;'
lines+=['END NETS','END DESIGN'];(P/'top.def').write_text('\n'.join(lines)+'\n')
