...
switch $stage {
  ...
  global_routing {
    # Keep the placed cells as a faint context behind the guides.
    gui::set_display_controls "Instances/*" visible true
...
set stage_dir [file join $here images [expr {$stage in {floorplan global_placement detailed_placement} ? "placement" : $stage in {cts global_routing detailed_routing} ? "routing" : "finishing"}]]
file mkdir $stage_dir
save_image -area $area -width 1400 [file join $stage_dir $stage.png]
if {$stage in {floorplan global_placement detailed_placement}} {
  # Exact row/cell boundaries for readable vector thumbnails.
  set file [open [file join $stage_dir ${stage}_geometry.tsv] w]
...
if {$stage eq "cts"} {
  ...
  set file [open [file join $stage_dir cts_geometry.tsv] w]
  puts $file "DIE\t[$die xMin]\t[$die yMin]\t[$die xMax]\t[$die yMax]"
  foreach inst [$block getInsts] {
    set box [$inst getBBox]
    puts $file "CELL\t[$inst getName]\t[[$inst getMaster] getName]\t[$box xMin]\t[$box yMin]\t[$box xMax]\t[$box yMax]"
