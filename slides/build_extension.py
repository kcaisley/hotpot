...
P=Path(__file__).resolve().parent;A=P/'images/extension';E=P/'examples/extension'
...
def gate_view(filename,master):
 ...
 import xml.etree.ElementTree as ET
root=ET.parse(P/'images'/filename).getroot()
 w=0.45*398.3386/(0.81*227.62207/650)
 h=w*float(root.get('height'))/float(root.get('width'))
 y=(650-h-55)/2
return [image(P/'images'/filename,0,y,w,h),text(w/2,y+h+42,master,30,G,'middle')]
def cell(kind='plain'):
